...
def _(load_dataset):
    # 部分的に取得するとき(スライス)
    spliced_train_datasets = load_dataset("madao33/new-title-chinese",split="train[:100]")
...
def _(load_dataset):
    # 部分的に取得するとき(パーセンテージ)
    pct_train_datasets = load_dataset("madao33/new-title-chinese",split="train[:50%]")
    pct_train_datasets
    return
...
def _(load_dataset):
    # 同じ種類のデータセットでも再分割できる
    hh_train_datasets = load_dataset("madao33/new-title-chinese",split=["train[:50%]","train[50%:]"])
    hh_train_datasets
    return
...
def _(boolq_datasets):
    ...
    boolq_datasets["train"].train_test_split(test_size=0.1,stratify_by_column="label")
    return
...
@app.cell
def _(load_dataset):
    csv_datasets = load_dataset("csv",data_files="./data/ChnSentiCorp_htl_all.csv")
    csv_datasets
    return
